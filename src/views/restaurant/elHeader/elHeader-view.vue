  ...
  methods: {
    ...
    SetOptionRestNumData() {
      getRestaurantPieChartsNum().then(res => {
        console.log('我是餐馆评论数量饼图');
        console.log(res.data.numberList[0]);
        let self = this;
        res.data.numberList[0].map(obj => {
          this.data1.push({
            name: obj.name,
            value: Math.round((obj.value / self.returnSum(res.data.numberList[0])) * 100)
          });
        });
        // res.data.numberList[0].name.map((item, index) => {
        //   this.data1.push({
        //     name: item,
        //     value: Math.round(
    ...
    SetOptionRestScoreData() {
      ...
      getRestaurantPieChartsScore().then(res => {
        res.data.scoreList[0].map(obj => {
          this.data2.push({
            name: obj.name,
            value: Math.round((obj.value / self.returnSum(res.data.scoreList[0])) * 100)
          });
        });
      });
    },
